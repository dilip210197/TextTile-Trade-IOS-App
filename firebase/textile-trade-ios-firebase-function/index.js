...
//     return {
//         firebase: {
//           databaseURL: 'https://textile-trade-50d88.firebaseio.com/',
//         }
//     };
// }
...
console.log("send message notification  >>> "+JSON.stringify(getSenderInfo));
  //determine if user is blocked
  const isUserBlocked = admin.database().ref(`blockedUsers/${toId}/${fromId}/`).once('value');

  const getUserName = admin.database().ref(`users/${fromId}/username`).once('value');


  //Execute the Functions
  return Promise.all([getDeviceTokensPromise, getSenderInfo, isUserBlocked,getUserName]).then(results => {
    const tokensSnapshot = results[0];
    const senderInfo = results[1];
    const isBlockedSnapshot = results[2];

    const username1 = results[3];
    ...
    const message = getMessagePayload(type, val, senderPhone, content, timestamp, fromId, toId, undefined, messageId, metadata,username);
...
exports.sendMessagesForGroups = functions.database.ref(`/groupsMessages/{groupId}/{messageId}`).onCreate((snap, context) => {
  ...
  const fromId = val.fromId;
  //console.log("sendMessagesForGroups >> fromId :: "+fromId);
  ///console.log("sendMessagesForGroups >> fromId type :: "+ typeof fromId);
  //message Details
  const content = val.content;
  const metadata = val.metadata;
  const timestamp = val.timestamp;
  ...
  const getGroupName = admin.database().ref(`groups/${groupId}/info/name`).once('value');

  const getTokens = admin.database().ref(`groups/${groupId}/info/name`).once('value');

  return Promise.all([getGroupName]).then(results => {

  
    let groupNameFinal = results[0].val();
  

  console.log("groupNameFinal :: ",groupNameFinal);


  //var message = getMessagePayload(type, val, fromPhone, content, timestamp, fromId, groupId, true, messageId, metadata,groupNameFinal);
  var message = getMessagePayload(type, val, fromPhone, content, timestamp, fromId, groupId, true, messageId, metadata,groupNameFinal);
  ...
  //console.log("conditions >>> "+JSON.stringify)
  message.condition = condition
  //send message to the givern groupId
 
  //console.log("sendMessagesForGroups >> Group ID :: "+groupId);
  return admin.database().ref(`groups/${groupId}/users`).once('value').then((usersSnap) => {
    
    const users = Object.keys(usersSnap.val())
    //console.log("sendMessagesForGroups >> getting group users :: "+JSON.stringify(users));
    const messagesToSave = []



    users.forEach(uid => {
      //console.log("sendMessagesForGroups >> itterating Users >> uid type :: "+ typeof uid);
      //in case if this message was deleted by this user
      if (uid !== fromId) {
        //console.log("sendMessagesForGroups >> itterating Users >> if condition");
    ...
    return Promise.all(messagesToSave).then(() => {
      message.data = removeExtraLimitForPayload(message.data)
      console.log("message to save mysqlog >> "+JSON.stringify(message.data));
      var messageFlag = admin.messaging().send(message)
      //print("meessage >>"+messageFlag)
      return messageFlag
    })
...
exports.participantAdded = functions.database.ref(`/groups/{groupId}/users/{userId}`).onCreate((snap, context) => {
  ...
  return admin.database().ref(`groups/${groupId}/info/createdBy`).once('value').then((authId) => {
    // console.log("auth id >>> "+JSON.stringify(authId))
    // console.log("auth id wwww >>> "+(authId))
    // console.log("auth id wwww val  >>> "+authId.val())
    const addedByUid = authId.val()
   // console.log("usersnap >>> "+addedByUid)
  ...
  //get userPhone
  const userPhonePromise = admin.database().ref(`/users/${userId}/username`).once('value')
  //get admin who added the user phone's 
  const addedByPhonePromise = admin.database().ref(`/users/${addedByUid}/username`).once('value')
...
console.log("results >>> "+JSON.stringify(results))
    ...
    if (userPhone === addedByPhone) {
       console.log("inside if  >>> "+snap.val())
  ...
  //const addedByUid = authId
//console.log("added by  >>> "+JSON.stringify(addedByUid))
...
exports.participantRemoved = functions.database.ref(`/groups/{groupId}/users/{userId}`).onDelete((snap, context) => {
  ...
  return admin.database().ref(`groups/${groupId}/info/createdBy`).once('value').then((authId) => {
    const deletedByUid = authId.val()
     //get removed user phone
    const userPhonePromise = admin.database().ref(`/users/${userId}/username`).once('value')
    //get phone of the admin who removed this user
    const removedByPhonePromise = admin.database().ref(`/users/${deletedByUid}/username`).once('value')
...
exports.groupInfoChanged = functions.database.ref(`/groups/{groupId}/info`).onUpdate((change, context) => {
  ...
  return admin.database().ref(`groups/${groupId}/info/createdBy`).once('value').then((authId) => {
    const changedById = authId.val()
    return admin.database().ref(`/users/${changedById}/username`).once('value').then((snapshot) => {
...
exports.sendMessageToBroadcast = functions.database.ref(`broadcastsMessages/{broadcastId}/{messageId}`).onCreate((snap, context) => {
  ...
  return Promise.all([getSenderInfo, isUserBlocked]).then(results => {
    ...
    return admin.database().ref(`broadcasts/${broadcastId}/users`).once('value').then((usersSnap) => {
      ...
      return Promise.all(messagesToSave).then(() => {
        message.data = removeExtraLimitForPayload(message.data)
        console.log("broadcasting data >> "+JSON.stringify(message.data))
        return admin.messaging().send(message)
      })
...
function getMessagePayload(type, val, senderPhone, content, timestamp, fromId, toId, isGroup, messageId, metadata,username = '') {
  var payload;
  ...
  //const getUserName = admin.database().ref(`users/${fromId}/username`).once('value');
  // let getUserName = admin.database().ref(`/users/${fromId}/username`).once('value').then((senderusername) => {
  //   console.log("username >> "+senderusername.val())
  //   //var username = senderusername.val()
  //   return senderusername.val();
  // })
  //   console.log("out username >> "+JSON.stringify(getUserName))
    var message = {
      data: data,
      "android": {
        "priority": "high"
    ...
    console.log("returning payload >>> "+JSON.stringify(message))
    return message
